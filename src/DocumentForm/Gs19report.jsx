...
function Gs19report() {
  ...
  const [projectKnowledgeExamDate, setProjectKnowledgeExamDate] = useState("");
  ...
  const handleConfirmSubmit = async (e) => {
    ...
    const requestData = {
      ...
      date_gs19report: datepicker,
      thesisAdvisor_gs19report: thesisAdvisor,
      semesterAt_gs19report: semesterAt,
      academicYear_gs19report: academicYear,
      courseCredits_gs19report: courseCredits,
      cumulativeGPA_gs19report: cumulativeGPA,
      projectKnowledgeExamDate_gs19report: projectKnowledgeExamDate,
      projectDefenseDate_gs19report: projectDefenseDate,
      additionalDetails_gs19report:
        additionalDetails.trim() === "" ? null : additionalDetails,
      signature_gs19report: signature,
      signName_gs19report: signName,
    };

    console.log("Request Data for Submission:", requestData);

    const response = await fetch(
      "http://localhost/TestPHP-API2/backend/submit_gs19report.php",
      {
        method: "POST",
        headers: {
    ...
    try {
      ...
      console.log("Response from submit_gs19report:", responseData);
  ...
  return (
    <>
      ...
      <div className="container-gsReport">
        <h3 style={{ margin: 40 }}>
          คคอ. บว. 19 แบบขออนุมัติผลการสำเร็จการศึกษา
          <br />
          นักศึกษาระดับปริญญาโท แผน 2 แบบวิชาชีพ
        </h3>
        <form onSubmit={handleSubmit} noValidate>
          ...
          <div className="input-group mb-3">
            ...
            <input
              type="date"
              className="form-control"
              id="projectKnowledgeExamDate"
              value={projectKnowledgeExamDate}
              onChange={(e) => setProjectKnowledgeExamDate(e.target.value)}
              name="date"
              required
            />
          </div>
          ...
          <div>
            <div className="input-group mb-3">
              <span className="input-group-text">
                ชื่ออาจารย์ที่ปรึกษาการศึกษาค้นคว้าอิสระหลัก
              </span>
              <select
                className="form-select"
                value={thesisAdvisor}
