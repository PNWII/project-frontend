...
function Gs13report() {
  ...
  const [revisionDateAdvisor, setRevisionDateAdvisor] = useState("");
  ...
  const [docProjectdetailsGs21rp, setDocProjectdetailsGs21rp] = useState(null);
  ...
  const handleReset = () => {
    ...
    setAdvisorMain("");
    setAdvisorSecond("");
    setRevisionDateAdvisor("");
    setsingName("");
    setDocProjectdetailsGs21rp(null);
  ...
  const handleConfirmSubmit = async () => {
    ...
    if (selectedFile) {
      ...
      try {
        ...
        if (uploadResponse.data.success) {
          ...
          setDocProjectdetailsGs21rp(filePath);
        } else {
    ...
    try {
      const submitResponse = await fetch(
        "http://localhost/TestPHP-API2/backend/submit_gs13report.php",
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            ...
            projectType_gs13report: projectType,
            projectThai_gs13report: projectThai,
            projectEng_gs13report: projectEng,
            revisionDateAdvisor_gs13report: revisionDateAdvisor,
            date_gs13report: datepicker,
            advisorMain_gs13report: advisorMain,
            advisorSecond_gs13report: advisorSecond,
            docProjectdetailsGs21rp_gs13report: filePath,
            signature_gs13report: signature,
            signName_gs13report: signName,
          }),
        }
      );
    ...
    <>
      ...
      <div className="container-gsReport">
        <h3 style={{ margin: 40 }}>
          คคอ. บว. 13 แบบขอส่งโครงการวิทยานิพนธ์/การศึกษาค้นคว้าอิสระ ฉบับแก้ไข{" "}
          <br />
          และ คคอ. บว. 21 แบบฟอร์มเสนอโครงการ
        </h3>
        <form onSubmit={handleSubmit} noValidate>
          ...
          <div className="input-group mb-3">
            <label className="input-group-text">
              มีความประสงค์ขอส่งโครงการ (ฉบับแก้ไข)
            </label>
            <select
          ...
          <div className="input-group mb-3">
            ...
            <input
              type="date"
              className="form-control"
              id="revisionDateAdvisor"
              value={revisionDateAdvisor}
              onChange={(e) => setRevisionDateAdvisor(e.target.value)}
              name="date"
              required
            />
          </div>
      ...
      <Modal show={showConfirmModal} onHide={() => setShowConfirmModal(false)}>
        ...
        <Modal.Body>
          ...
          <p>
            1. ให้อาจารย์ที่ปรึกษาวิทยานิพนธ์/การศึกษาค้นคว้าอิสระ
            ลงนามบนปกหน้าของเอกสารโครงการวิทยานิพนธ์ (คคอ. บว. 13) ฉบับแก้ไขด้วย
            <br />
            2. โครงการวิทยานิพนธ์/การศึกษาค้นคว้าอิสระ
            ฉบับแก้ไขต้องดำเนินการให้แล้วเสร็จภายใน 30 วัน
            นับจากวันสอบหัวข้อวิทยานิพนธ์/การศึกษาค้นคว้าอิสระ
          </p>
        </Modal.Body>
        <Modal.Footer>
