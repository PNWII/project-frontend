...
function Gs12report() {
  ...
  const [examRequestDate, setExamRequestDate] = useState("");
  const [examRequestTime, setExamRequestTime] = useState("");
  const [examRequestRoom, setExamRequestRoom] = useState("");
  const [examRequestFloor, setExamRequestFloor] = useState("");
  const [examRequestBuilding, setExamRequestBuilding] = useState("");
  ...
  const [docProjectdetailsGs20rp, setDocProjectdetailsGs20rp] = useState(null);
  ...
  const handleReset = () => {
    ...
    setExamRequestRoom("");
    setExamRequestFloor("");
    setExamRequestBuilding("");
    setAdvisorMain("");
    setAdvisorSecond("");
    setsingName("");
    setDocProjectdetailsGs20rp(null);
  ...
  const handleConfirmSubmit = async () => {
    ...
    if (selectedFile) {
      ...
      try {
        ...
        if (uploadResponse.data.success) {
          ...
          setDocProjectdetailsGs20rp(filePath);
        } else {
    ...
    try {
      const submitResponse = await fetch(
        "http://localhost/TestPHP-API2/backend/submit_gs12report.php",
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            ...
            projectType_gs12report: projectType,
            projectThai_gs12report: projectThai,
            projectEng_gs12report: projectEng,
            examRequestDate_gs12report: examRequestDate,
            examRequestTime_gs12report: examRequestTime,
            examRequestRoom_gs12report: examRequestRoom,
            examRequestFloor_gs12report: examRequestFloor,
            examRequestBuilding_gs12report: examRequestBuilding,
            date_gs12report: datepicker,
            advisorMain_gs12report: advisorMain,
            advisorSecond_gs12report: advisorSecond,
            docProjectdetailsGs20rp_gs12report: filePath,
            signature_gs12report: signature,
            signName_gs12report: signName,
          }),
        }
      );
  ...
  return (
    <>
      ...
      <div className="container-gsReport">
        <h3 style={{ margin: 40 }}>
          คคอ. บว. 12 แบบขอสอบหัวข้อวิทยานิพนธ์/การศึกษาค้นคว้าอิสระ <br />
        ...
        <form onSubmit={handleSubmit} noValidate>
          ...
          <div className="input-group mb-3">
            <label className="input-group-text">
              มีความประสงค์ขอสอบหัวข้อในวันและเวลา ดังนี้
            </label>
            ...
            <input
              ...
              className="form-control"
              id="examRequestDate"
              value={examRequestDate}
              onChange={(e) => setExamRequestDate(e.target.value)}
              name="datetime"
              required
            />
            ...
            <input
              ...
              className="form-control"
              id="examRequestTime"
              value={examRequestTime}
              onChange={(e) => setExamRequestTime(e.target.value)}
              name="datetime"
              required
            />
          </div>
          <div className="input-group mb-3">
            <label className="input-group-text">
              สถานที่ขอสอบหัวข้อ ณ ห้อง
            </label>
            <input
              type="text"
              name="examRequestRoom"
              className="form-control"
              placeholder="กรุณากรอกชื่อหรือหมายเลขห้อง"
              id="examRequestRoom"
              value={examRequestRoom}
              onChange={(e) => setExamRequestRoom(e.target.value)}
              required
            />
            ...
            <input
              type="text"
              id="examRequestFloor"
              name="examRequestFloor"
              value={examRequestFloor}
              onChange={(e) => setExamRequestFloor(e.target.value)}
              className="form-control"
              placeholder="กรุณากรอกชั้นที่ขอสอบหัวข้อ"
              required
            />
            <label className="input-group-text">อาคาร</label>
            <input
              type="text"
              id="examRequestBuilding"
              name="examRequestBuilding"
              value={examRequestBuilding}
              onChange={(e) => setExamRequestBuilding(e.target.value)}
              className="form-control"
      ...
      <Modal show={showConfirmModal} onHide={() => setShowConfirmModal(false)}>
        ...
        <Modal.Body>
          ...
          <p>
            การสอบหัวข้อวิทยานิพนธ์/การศึกษาค้นคว้าอิสระต้องสอบภายใน 30 วัน
            นับจากวันที่ได้รับอนุมัติโครงการวิทยานิพนธ์/การศึกษาค้นคว้าอิสระ
            <br />
            มิฉะนั้นจะต้องเสนอโครงการวิทยานิพนธ์/การศึกษาค้นคว้าอิสระใหม่
          </p>
        </Modal.Body>
        <Modal.Footer>
